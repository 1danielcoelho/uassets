...
- **Fix development workflow**: I'm using `bun run dev` to run the dev workflow. Sometimes some of the changes you do don't quite "refresh" the webpage properly? I wonder if bun is not detecting all file changes somehow, or if it's not capable of doing a full rebuild after every change or something like that? Because sometimes I have to fully stop the dev server and restart it and then refresh the page to get the full changes
