...
- **Support for asset-specific data**: We should only cover this for some basic asset types, but it would be nice if at least for e.g. StaticMesh, Material, MaterialInstance, Blueprint, Texture and Level we could step into their custom serialization code and try to parse what their asset-specific data looks like (e.g. Mesh description bulk data, maybe there are some DDC keys in there, etc.). Ideally after we've done the previous task and then this one we would be able to annotate *every single byte* in the sample files
  - This has been done for StaticMeshes, materials and textures so far. We have also only parsed the package trailer stuff, as we still need to look at a cooked assets though
...
- **Manually harvest all GUIDs**: I have the source code here, so I should be able to display what each custom version corresponds to by checking the guid declarations on the source code. Maybe instead of having it say just "Name [0]" it should say "FortniteCustomVersion" or whatever the GUID corresponds to?
...
- **Display enums as strings on the annotation view**: Whenever we have enum values deserialized, we should display the string value too (e.g. "3 (EMethod::Oodle)" instead of just "3")
